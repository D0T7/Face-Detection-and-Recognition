...
ap.add_argument("-i", "--dataset", required=True,
                help="here shows path to input directory of faces + images")
ap.add_argument("-e", "--embeddings", required=True,
                help="here shows path to output serialized db of facial embeddings")
...
print("[INFO] here quantifying faces...")
...
for (i, imagePath) in enumerate(imagePaths):
    print("[INFO] now processing image {}/{}".format(i + 1,
                                                     len(imagePaths)))
    ...
    if len(detections) > 0:
        ...
        if confidence > args["confidence"]:
            ...
            total += 1
print("[INFO] now serializing {} and encodings...".format(total))
